IF EXISTS (
		SELECT *
		FROM sys.objects
		WHERE object_id = OBJECT_ID(N'SelectInterNewsToApprove') AND type IN (N'P', N'PC')
		)
...
BEGIN
	...
	BEGIN CATCH
		...
		VALUES ('Database Error', 'SelectInterNewsToApprove', 'Error from SelectInterNewsToApprove Store Procedure', ERROR_NUMBER(), ERROR_MESSAGE())
	END CATCH
END
